...
local function show_dialog(unit, item_image)
	if cc_utils.global_vars.skip_pickup_dialog then
		return 1
	end

	local dialog_wml = wml.load "campaigns/Custom_Conquest/gui/pickup_confirmation.cfg"
	local function preshow(dialog)
		dialog.unit_icon.label = cc_color.tc_image(wesnoth.unit_types[unit.type].image)
		dialog.arrow.label = cc_color.tc_text(" → ")
