...
function wesnoth.effects.cc_optional_attack(u, cfg)
	local name_suffix = cfg.name_suffix or wml.error("apply_to=cc_optional_attack missing required name_suffix= attribute.")
	local attack_mod = wml.get_child(cfg, "attack") or wml.error("apply_to=cc_optional_attack missing required [attack] subtag")
...
function wesnoth.effects.cc_moves_defense(u, cfg)
...
function wesnoth.effects.cc_min_resistance(u, cfg)
...
-- inverts the negative resistances of something
function wesnoth.effects.cc_invert_resistance(u, cfg)
	local resistance_new = {}
	local resistance_old = wml.get_child(u.__cfg, "resistance")
	for k,v in pairs(resistance_old) do
		if type(k) == "string" and type(v) == "number" then
			if v > 100 then
				resistance_new[k] = 200-v
			end
		end
	end
	wesnoth.units.add_modification(u, "object", {
...
function wesnoth.effects.cc_min_defense(u, cfg)
	local defense_new = {}
...
function wesnoth.effects.cc_update_aura(u, cfg)
	...
	if illuminates and darkens then
		wesnoth.interface.add_chat_message("CC", "Warning illuminates and darkens discovered on a unit")
	end
	if darkens then
...
function wesnoth.effects.cc_overlay(u, cfg)
	if cfg.add then
...
function wesnoth.effects.cc_recall_cost(u, cfg)
