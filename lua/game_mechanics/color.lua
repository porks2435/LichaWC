...
-- scaling greens 
function color.intensity_text(str,max_value)

	if tonumber(str) <= max_value * 0.20 then
		return color.color_text("#caff00", str)
	elseif tonumber(str) <= max_value * 0.4 then
		return color.color_text("#95ff00", str)
	elseif tonumber(str) <= max_value * 0.6 then
		return color.color_text("#60ff00", str)
	elseif tonumber(str) <= max_value * 0.8 then
		return color.color_text("#2bff00", str)
	else
		return color.color_text("#00df00", str)
	end
end
...
function wesnoth.wml_actions.cc_fix_colors(cfg)
	...
	for i, side in ipairs(player_sides) do
		...
		if side.side == 4 and taken_colors[side.color] then
			...
			if side.variables.cc_color then
				side.color = side.variables.cc_color
			else
				side.variables.cc_color = side.color
			end
