...
local function cc_map_custom_ruin_village(loc)
...
on_event("die", function(ec)
	local map = wesnoth.current.map
	local loc = wesnoth.map.get(ec.x1, ec.y1)
	if wml.variables.cc_config_enable_terrain_destruction == false then
		return
	end
	...
	elseif loc:matches{terrain = "*^Vh,**^Vhc,*^Vha,**^Vhca,*^Fda"} then
		cc_map_custom_ruin_village(loc)
