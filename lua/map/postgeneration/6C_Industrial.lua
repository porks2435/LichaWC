-- Industrial
-- TODO: this is somewhat slow, maybe it because it used cc_iterate_road_to ?
local function cc_conect_factory_rails()
...
local function cc_store_possible_dirty_delta()
...
local function cc_dirty_deltas()

	local terrain_to_change = cc_store_possible_dirty_delta()
	while #terrain_to_change > 0 do
		...
		map[loc] = ter
		terrain_to_change = cc_store_possible_dirty_delta()
	end
end

local function cc_store_possible_ford_delta()
	return map:find(f.all(
		f.terrain("Wwf"),
...
local function cc_ford_deltas()

	local terrain_to_change = cc_store_possible_ford_delta()
	while #terrain_to_change > 0 do
		...
		map[loc] = ter
		terrain_to_change = cc_store_possible_ford_delta()
	end
end


local function cc_rails_to_industrial_keep(radius)
	-- "Cud^Br|"
	return map:find(f.all(
		f.terrain("C*"),
		f.adjacent(f.all(
...
local function cc_roads_to_industrial_village(radius)
	-- "Rb"
	return map:find(f.all(
...
local function cc_roads_to_industrial_city(radius)
	-- "Rrc"
	return map:find(f.all(
		f.terrain("!,W*^*,*^V*,*^Bcx*,Urb,C*,K*^*,R*"),
		f.adjacent(f.all(
...
local function cc_roads_to_factory(radius)
	--"Rr"
	return map:find(f.all(
		f.terrain("!,W*^*,*^V*,*^Bcx*,Urb,C*,K*^*,R*"),
		f.adjacent(f.all(
...
local function cc_roads_to_river_industry(radius)
	-- "Re"
	return map:find(f.all(
		f.terrain("!,W*^*,*^V*,*^Bcx*,Urb,C*,K*^*,R*"),
		f.adjacent(f.all(
...
local function cc_industry_bridge(bridge, directions)
...
local function world_conquest_tek_map_decoration_6c()
	...
	cc_industry_bridge("\\", "nw,se")
	cc_industry_bridge("/", "ne,sw")
	cc_industry_bridge("|", "n,s")
	cc_iterate_roads_to(cc_roads_to_river_industry, 3, "Re")
	set_terrain { "Urb",
		f.terrain("Re,Rp"),
	}
	set_terrain { "*^Bcx\\",
	...
	cc_iterate_roads_to(cc_roads_to_river_industry, 4, "Re")
	set_terrain { "Urb",
	...
	cc_iterate_roads_to(cc_roads_to_factory, 4, "Rr")
	set_terrain { "Rr^Vhh",
		f.terrain("G*^Vhh"),
	}
	set_terrain { "Hhd^Vhh",
	...
	cc_iterate_roads_to(cc_roads_to_industrial_city, 5, "Rrc")
	-- sawmills
	set_terrain { "*^Vl",
		f.all(
	...
	cc_iterate_roads_to(cc_roads_to_industrial_village, 3, "Rb")
	-- castle rails
	set_terrain { "Rr^Br|",
		f.all(
			f.terrain("Rr"),
	...
	cc_iterate_roads_to(cc_rails_to_industrial_keep, 3, "Cud^Br|")
	set_terrain { "Rr^Br\\",
		f.all(
...
local function world_conquest_tek_map_repaint_6c()
	cc_map_reduce_castle_expanding_recruit("Ce", "Wwf")
	set_terrain { "Ch",
		f.terrain("Ce"),
	}
	...
	cc_reduce_wall_clusters("Uu,Uu^Tf,Uh,Uu^Tf,Uu,Uu^Tf,Uh,Ql,Qxu,Xu,Uu,Rb")
	cc_fill_lava_chasms()
	cc_volcanos()
	cc_volcanos_dirt()
