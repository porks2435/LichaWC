...
function world_conquest_tek_map_decoration_1()
	...
	-- chances of fords
	local terrain_to_change = cc_store_possible_encampment_ford();

	while #terrain_to_change > 0 and mathx.random(2) == 1 do
		local i = mathx.random(#terrain_to_change)
		map[terrain_to_change[i]] = "Wwf"
		terrain_to_change = cc_store_possible_encampment_ford()
	end

	if mathx.random(20) ~= 1 then
		cc_change_map_water("g")
	end
...
function cc_map_1_post_castle_expansion_fix()
...
function cc_store_possible_encampment_ford()
...
return function()
	...
	cc_map_1_post_bunus_decoration()

	-- the original code did the choose difficulty dialog here.
	-- claiming it would ne needed before cc_enemy_castle_expansion
	-- but i don't know why.
	cc_enemy_castle_expansion()
	cc_map_1_post_castle_expansion_fix()
