...
function world_conquest_tek_map_repaint_3f()
	...
	cc_reduce_wall_clusters("Uu,Uu^Tf,Uh,Uu^Tf,Uu,Uh,Uu,Uu,Qxu,Uu,Wwf")

	-- soft rough terrain generated
	local terrain_to_change = cc_store_cave_passages_candidates()
	while #terrain_to_change > 0 do
		...
		terrain_to_change = cc_store_cave_passages_candidates()
	end
	set_terrain { "Gd",
		f.terrain("Mm"),
		fraction = 4,
...
function cc_map_3f_post_bunus_decoration(bonus_points)
	-- add fords
	set_terrain { "Wwf",
		f.all(
			f.terrain("G*"),
...
function cc_store_cave_passages_candidates()
...
return function()
	...
	cc_map_3f_post_bunus_decoration(bonus_points)
	cc_map_enemy_themed("human", "Young Ogre", "e", "Gg^Vl", 8)
end
