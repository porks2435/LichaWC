...
function cc_map_2e_post_bunus_decoration()
	cc_map_decoration_3e_keeps()
	cc_map_decoration_3e_leantos()
end
...
function world_conquest_tek_map_repaint_2e()
	...
	cc_map_reduce_castle_expanding_recruit("Xos", "Rr^Fet")
	set_terrain { "Ch",
		f.terrain("Xos"),
	}
	-- create mini lakes
	...
	for i = 1, nyards do
		...
		cc_map_yard(yard_dir, yard_cdir)
	end
...
function cc_map_yard(directions, counter_directions)
...
function cc_conect_isolated_citadel()
...
function cc_store_empty_citadel()
...
function cc_map_decoration_3e_keeps()
	set_terrain { "Ch,Kh",
		f.all(
			f.terrain("Ch"),
...
function cc_map_decoration_3e_leantos()
	local terrain_to_change = map:find(f.all(
		f.terrain("Rr"),
		f.none(
...
return function()
	...
	cc_map_2e_post_bunus_decoration()
end
