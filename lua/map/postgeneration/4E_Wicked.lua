...
function world_conquest_tek_map_repaint_4e()
	cc_reduce_wall_clusters("Uu,Uu,Uu,Uu,Uu,Uu,Uu,Uu,Uh,Uh,Uh,Uh,Uu^Tf,Wwf,Ur,Qxu,Ql,Wwg")
	cc_fill_lava_chasms()
...
function cc_map_4e_post_bunus_decoration()
	-- store map size and create axi filters variables
	local f_north_half = f.x("1-" .. (map.height // 2))
	local f_south_half = f.none(f_north_half)
	local f_west_half = f.y("1-" .. (map.width // 2))
...
return function()
	...
	cc_map_4e_post_bunus_decoration()
	...
	cc_map_enemy_themed("celestial", "Jinn", "e", "Gg^Vl", 1)
	cc_map_enemy_themed("harpies", "Elder Falcon", "v", "Gg^Ve", 1)
end
