...
function roads_to_dock(radius)
	...
	--old implementation: cc_iterate_roads_to(cc_roads_to_dock, 3, "Rp")
end

function cc_roads_to_dock(radius)
...
function roads_to_river(radius)
	...
	--old implementation: cc_iterate_roads_to(cc_roads_to_river, 3, "Rp")
end

-- todo: the old code used
-- `cc_iterate_roads_to(cc_roads_to_river, 3, "Rp")`
-- but the new code uses roads_to_river(4) i guess
-- cc_iterate_roads_to_ex defeind radiosu differently thatn
-- cc_iterate_roads_to ?
-- anyway leavong this function in as a sekeltong on how
-- cc_iterate_roads_to worked. in particular if we want to convert the remaining cases to
-- cc_iterate_roads_to_ex
function cc_roads_to_river(radius)
...
function world_conquest_tek_map_decoration_6b()
	...
	cc_map_reduce_castle_expanding_recruit("Chw", "Wwf")
...
function world_conquest_tek_map_repaint_6b()
	cc_reduce_wall_clusters("Uu,Uu^Tf,Uh,Uu^Tf,Uu,Uu^Tf,Uh,Ql,Qxu,Xu,Uu,Ur")
	cc_fill_lava_chasms()
	cc_volcanos()
