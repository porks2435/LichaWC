...
function world_conquest_tek_map_decoration_2a()
	...
	-- chances flowers
	local terrain_to_change = cc_store_possible_flowers("G*^Fet")
	while #terrain_to_change > 0 and mathx.random(10) ~= 1 do
		local loc = terrain_to_change[mathx.random(#terrain_to_change)]
		map[loc] = "^Efm"
		terrain_to_change = cc_store_possible_flowers("G*^Fet")
	end
	-- extra coast
	...
	cc_map_reduce_castle_expanding_recruit("Ce", "Re")
	if mathx.random(5) ~= 1 then
		cc_map_decorative_docks()
	end
end

function cc_map_2a_post_bunus_decoration()
...
return function()
	...
	cc_map_2a_post_bunus_decoration()
	cc_map_enemy_themed("undead", "Soulless", "ha", "Aa^Vha", 12)
end
