...
local function add_enemy_side(scenario, gold, starting_pos)
	...
	-- recruits and leader gets overwritten later by [cc_enemy] implementation
	local side = {
		wml.tag.ai {
...
local function add_player_side(scenario, scenario_num, gold)
	...
	local side = {
		side = side_num,
		type = "Peasant",
		id = "cc_leader" .. side_num,
		save_id = "cc_leader" .. side_num,
		persistent = true,
		canrecruit = true,
		gold = gold,
		controller = "human",
	...
	if side_num == wml.variables.cc_defeated_side then
		side.save_id = "cc_leader4"
		side.id = "cc_leader4"
	end
	table.insert(scenario.side, side)
end
...
function cc_ii_generate_sides(scenario, prestart_event, nplayers, scenario_num, enemy_data, scenario_data)
	...
	for i = 1, n_enemy_sides do
		...
		--note: this must go before the 'cc_enemy_themed' generated by the mapgen.
		--note2: this must go after "difficulty selection"
		--note3: it doesn't
		table.insert(prestart_event, wml.tag.cc_enemy {
