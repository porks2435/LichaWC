...
function get_f_cc_bonus_location_filter(map)
	local scenario_num = wml.variables.cc_scenario or 1
	return f.all(
...
function cc_bonus_chose_scenery(loc, theme, filter_extra)
...
function world_conquest_tek_bonus_points(theme)
	...
	oceanic = get_oceanic()
	f_cc_bonus_location_filter = wesnoth.map.filter(get_f_cc_bonus_location_filter(map), { oceanic = oceanic })
	local possible_locs = map:find(f_cc_bonus_location_filter)
	function place_item(loc)
		scenery = cc_bonus_chose_scenery(loc, theme, { oceanic = oceanic })
		table.insert(prestart_event, wml.tag.cc_place_bonus {
			x= loc[1],
			y= loc[2],
			scenery = scenery,
		})
