...
function cc_dirt_beachs(rng_range)
	set_terrain { "Ds^Esd",
		f.all(
			f.terrain("Ds"),
...
function world_conquest_tek_map_dirt(mushrooms)
	...
	local terrain_to_change = cc_store_possible_muddy_swamps()
	while #terrain_to_change > 0 do
		for i, v in ipairs(terrain_to_change) do
			map[v] = "Sm"
		end
		terrain_to_change = cc_store_possible_muddy_swamps()
	end

	cc_randomize_snowed_forest()
	cc_volcanos_dirt()
...
function cc_change_map_water(t)
	set_terrain { "Ww" .. t,
		f.terrain("Ww^*"),
		layer = "base",
	}
...
function cc_map_decorative_docks()
	set_terrain { "Ww^Bw/",
		f.all(
			f.terrain("Ww"),
			f.adjacent(f.all(
...
function cc_store_possible_flowers(terrain)
...
function cc_store_possible_map4_castle(value)
	return  map:find(f.all(
		f.terrain("H*^F*"),
...
function cc_possible_map4_castle(terrain, value)
	local terrain_to_change = cc_store_possible_map4_castle(value)
	while #terrain_to_change > 0 and mathx.random(value + 1) == 1 do
		local loc = terrain_to_change[mathx.random(#terrain_to_change)]
		map[loc] = terrain
		terrain_to_change = cc_store_possible_map4_castle(value)
	end
end

function cc_store_possible_dwarven_castle()
	return map:find(f.all(
		f.terrain("Uh"),
...
function cc_store_possible_roads(village)
	return map:find(f.all(
		f.terrain("Gg,Gs"),
		f.adjacent(f.all(
			f.terrain(village),
...
function cc_road_to_village(road, village)
	-- build roads of 1 hex to conect villages
	local terrain_to_change = cc_store_possible_roads(village)
	while #terrain_to_change > 0 do
		local loc = terrain_to_change[mathx.random(#terrain_to_change)]
		map[loc] = road
		terrain_to_change = cc_store_possible_roads(village)
	end
end

-- builds roads using terrain @a terrain
-- for each step we call @get_next
function cc_iterate_roads_to(get_next, radius, terrain)
	print_time("cc_iterate_roads_to start")
	for r = radius, 1, -1 do
	...
	print_time("cc_iterate_roads_to end")
end

-- creates roads, faster than cc_iterate_roads_to
...
function cc_iterate_roads_to_2(f_validpath, f_src, f_dest, terrain_road, radius)
...
function cc_iterate_roads_to_ex(t)
	print_time("cc_iterate_roads_to_ex start")
	cc_iterate_roads_to_2(t.f_validpath, t.f_src, t.f_dest, t.terrain_road, t.radius)
	print_time("cc_iterate_roads_to_ex end")
end
function cc_break_walls(wall, terrain)

	local terrain_to_change = cc_store_broken_wall_candidates(wall)
	while #terrain_to_change > 0 do
		local loc = terrain_to_change[mathx.random(#terrain_to_change)]
		map[loc] = mathx.random_choice(terrain)
		terrain_to_change = cc_store_broken_wall_candidates(wall)
	end
end

function cc_store_broken_wall_candidates(wall)
	return map:find(f.all(
		f.terrain(wall),
...
function cc_store_possible_muddy_swamps()
	return map:find(f.all(
		f.terrain("Ss"),
...
function cc_map_reduce_castle_expanding_recruit(castle, terrain)
...
function cc_map_cave_path_to(terrain)
	set_terrain { terrain,
		f.terrain("Ur"),
	}
