...
function cc_map_enemy_themed(race, pet, castle, village, chance)
	table.insert(prestart_event, wml.tag.cc_enemy_themed {
		race = race,
		pet = pet,
		castle = castle,
		village = village,
...
function cc_map_generator(default_id, postgen_id, length, villages, castle, iterations, hill_size, players, island)
	return function(scenario, nhumanplayer)
		wesnoth.log("debug", "cc_map_generator " .. default_id .. " " .. postgen_id)
