...
function get_defaults(nplayer, nscenario)
	if filesystem.have_file(string.format("./../scenarios/CC_%dp_scenario%d.lua", nplayer, nscenario)) then
		local old_cc_map_generator = _G.cc_map_generator
		_G.cc_map_generator = function(default_id, postgen_id, length, villages, castle, iterations, hill_size, players, island)
			return {
		...
		local scenario_data = wesnoth.dofile(string.format("./../scenarios/CC_%dp_scenario%d.lua", nplayer, nscenario))
		_G.cc_map_generator = old_cc_map_generator
		return scenario_data.generators
	end
	std_print(string.format("./../scenarios/CC_%dp_scenario%d.lua", nplayer, nscenario), " Not found")
	return {}
end

local dialog_wml = wml.load "campaigns/Custom_Conquest/gui/settings_dialog.cfg"

function cc_debug_settings()
