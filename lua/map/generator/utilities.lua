...
function cc_fix_river_into_ocean(t, height)
...
function cc_generator_settings_arguments(length, villages, castle, iterations, hill_size, players, island)
	local width = length
	if width % 2 == 1 then
		width = width + 1
	end
...
function cc_generator_village(GG, GS, DS, UU, UH, GSFP, HH, MM, AA, AAFPA, SS, WW)
	return flatten1 {
		dr_village {
			terrain="Gg",
			convert_to="Gg^Vh",
...
function cc_generator_road_cost_classic()
	return flatten1 {
		dr_road("Gg", "Re", 10),
		dr_road("Gs^Fp", "Re", 20),
		dr_road("Hh", "Re", 30),
...
function default_generate_map(data)
	...
	local cfg = cc_convert.lon_to_wml(data, "mg_main")
